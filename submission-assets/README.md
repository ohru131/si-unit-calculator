...
| `demo/unit-calculator-demo-en-silent.webm` | デモ動画（無音・英語字幕を焼き込み済み・**1分53秒**・1080×1800・25fps・VP8）。2026-09-22 に現行UIで録り直し済み（電卓とノートで共用する式キーボード＝ツール行 `定数`・`xⁿ`・`f(x)`・`αβ`・`ABC`・`単位`＋5列4段のキーパッド、カーソル `◀ ▶` はキーパッドの中、`=` はキーパッドから外れ、入力欄の右のボタンも `=` ではなく保存のアイコン、入力欄＝色分けトークン列＋常時表示のキャレット、アプリ名 UnitCalc）。**ファイル名だけは旧名のまま**で、`scripts/record-demo-video.mjs` の出力先と `submission-assets/README.md` の両方に書いてあるので、変えるなら両方を同時に直すこと |
...
**2026-09-22 にも同じことが起きた。しかも今回は2つぶん溜まっていた。** (1) 入力欄の右の確定ボタンが
`=` から**保存のアイコン**に変わったこと（結果は打つそばから出るので、このボタンがやるのは履歴に残す・
定数の定義を保存するだけ。読み上げラベルも `Result` から `Save calculation` に変わった）、空状態の
クイックスタートが `R = 4.7kΩ` になったこと、表示桁で切り詰めた値の下に切り詰めていない値を小さく
併記するようになったこと。そして (2) **#79 の「表示する桁数」（既定6桁）が一度も撮られていなかった**
——設定画面に `Digits shown` の行が増え、計算ノートの一覧プレビューが `370.3703704 A*h` から
`370.37 A*h` に変わっていたのに、前回の撮り直し（2026-09-20）が #79 より前だったため。
**14カット×6言語＋`06-notebook-list` の4枚＝88枚**が差し替わり、`05-library-grid` /
`07-notebooks-tab` / `09-pro` の18枚と `06-notebook-list` の2枚はバイト単位で同一だった。
`08-settings` と `13-notebook-search` が電卓のUI変更で変わったのは (2) のためで、下の
「電卓タブを写さない6カットは変わらない」という目安は**表示桁のように全画面に効く設定を足したときは
当てはまらない**。デモ動画も同じ理由で録り直した（**字幕SRTと台本の文言は変えていない**——
`=` への言及は台本の地の文だけで、字幕には元から無かった）。

**このとき `scripts/record-demo-video.mjs` が止まった。** `key(page, "=")` でキーパッドの `=` を押していたが、
`=` はキーパッドから消えて入力欄の右のボタンだけになったので、セレクタが30秒待って落ちる
（**素材が古くなるのとは別に、撮影スクリプト自体がUI変更で壊れる**）。確定は `submit(page)` に
一本化した。`=` は `ABC` パネルの中（定数定義用）にしか無いので、`key(page, "=")` に戻すと
パネルを開いていない限り必ず落ちる。**2026-09-22 にそのボタンがアイコンになったので、
`submit(page)` が引くラベルも `getByLabel("Result")` から `getByLabel("Save calculation")` へ
変わった**（`Result` は結果カードの見出しの方に残っている）。撮影スクリプト側は
`LABELS[lang].submit` を新設して言語ごとに持たせてある。
...
| **結果の出し方（表示桁・有効数字・丸め）** | **全カット**（ノート一覧のプレビューと設定にも出る）とデモ動画 |
...
**ただし「結果をどう出すか」の変更はこの目安の外**——2026-09-22 の撮り直しでは、表示桁の上限（#79）が
ノート一覧のプレビューと設定画面に出るため `06` `08` `13` も差し替わった。
