...
export default function ConstantsScreen() {
  ...
  const applyStepNameValue = (step: CalculationNoteStep, name: string, value: string) => {
    if (!name) { updateStep(step.id, { resultSymbol: undefined, expression: value }); return; }
    // 判定は詳細画面（components/notebooks/notebook-detail.tsx）と同じ関数を使う。両画面で式を
    // 別々に持つと、片方だけ直したときに挙動が分岐して気付けない。
    updateStep(step.id, { resultSymbol: name, expression: value, ...(isAutoGeneratedStepTitle(step.title, step.resultSymbol) ? { title: name } : {}) });
  };
