...
import { unitExamplesFromHistory } from "@/lib/unit-context-suggestions";
import {
  analyzeExpression,
  hasDivisionOperator,
...
// このアプリの売り（オームの法則で電圧が求まる・答えが読みやすい接頭語で返る・速さ×時間が距離になる・
// 定数を定義して後の式で使える）を1タップずつで見せるための固定4件。表示単位は指定せず、表示単位の
// 自動選択（lib/display-unit.ts）に任せる（1 V / 2.55 mA / 90 km になることが、そのまま自動選択のデモになる）。
// 4件目の定数定義は、かつて「?」のヘルプにしか書かれていなかった唯一の情報。ヘルプは開いても
// 入力欄へ繋がらない読み物だったので、そのままタップで試せるこの列へ移した。
const QUICK_START: { id: string; expression: string; hintKey: "quickStartOhmsLaw" | "quickStartCurrent" | "quickStartDistance" | "quickStartDefine" }[] = [
  { id: "ohms_law", expression: "1kΩ × 1mA", hintKey: "quickStartOhmsLaw" },
  { id: "current", expression: "12V / 4.7kΩ", hintKey: "quickStartCurrent" },
  { id: "distance", expression: "60km/h × 90min", hintKey: "quickStartDistance" },
  { id: "define", expression: "R = 4.7kΩ", hintKey: "quickStartDefine" },
];
 ...
 * 式の入力欄に描くキャレット（カーソル）。
 *
 * OSのキーボードを受けるTextInputは1×1の透明な要素として隠してあり、そもそもキャレットが
 * 見えない。加えてこの電卓は「入力欄を触らずキーパッドで式を組み立てる」設計なので、
 * フォーカスの有無に関わらずカーソル位置が常に見えている必要がある。だからアプリ側で描く。
 *
 * 点滅させるのは、静止した縦棒だと式の一部（`|` のような記号）と見分けられないため。
 * 点滅は reanimated の共有値だけで回し、setState を使わない（1秒ごとに再レンダーが走ると
 * 入力のたびに式の解析までやり直すことになる）。
 */
function ExpressionCaret({ colors }: { colors: ThemeColorPalette }) {
...
/**
 * 式の入力欄（色分けしたトークン列）の一片。タップされた位置から何文字目かを割り出す。
 *
 * 幅は `onLayout` で実測する。等幅フォントなので「一片の幅に対するタップ位置の割合 × 文字数」が
 * そのまま何文字目かになり、プラットフォームごとに違う等幅の字送り比率を係数として持たずに済む
 * （キャレットの位置を文字幅から計算せず、要素の間に挟み込むのと同じ理由）。
 */
function ExpressionPiece({ accessibilityLabel, children, length, onPlaceCaret, onPress, proportional, start, style }: {
  accessibilityLabel?: string;
  children: ReactNode;
  length: number;
  onPlaceCaret: (position: number) => void;
  onPress?: () => void;
  ...
  return (
    <Pressable
      ...
      onPress={(event) => {
        ...
        // react-native-web はマウスのクリックを DOM の click からそのまま onPress へ渡すので、
        // nativeEvent は MouseEvent そのもので locationX を持たない（ネイティブの
        // GestureResponderEvent だけが持つ）。Webでは同じ意味を持つ offsetX で補う。
        const native = event.nativeEvent as { locationX?: number; offsetX?: number };
        const x = typeof native.locationX === "number" ? native.locationX : native.offsetX;
        // 枠とアイコンを持つ一片（未対応単位）は要素の幅と文字の幅が一致しないので、割合から
        // 文字数を割り出さず末尾へ置く。文字だけの一片は等幅なので割合がそのまま文字数になる。
        const offset = proportional && width > 0 && typeof x === "number" && Number.isFinite(x)
          ? Math.round((Math.max(0, Math.min(width, x)) / width) * length)
          : length;
        onPlaceCaret(start + offset);
        onPress?.();
      }}
      style={style}
    >
...
const EN_COPY = {
  ...
  savedItemLoaded: "Saved item loaded.",
...
const COPY: Record<AppLanguage, typeof EN_COPY> = {
  ...
  ja: {
    ...
    savedItemLoaded: "保存した項目を読み込みました。",
  ...
  es: {
    ...
    savedItemLoaded: "Elemento guardado cargado.",
  ...
  "pt-BR": {
    ...
    savedItemLoaded: "Item salvo carregado.",
  ...
  de: {
    ...
    savedItemLoaded: "Gespeicherter Eintrag geladen.",
  ...
  fr: {
    ...
    savedItemLoaded: "Élément enregistré chargé.",
    ...
    quickStartDefine: "Définissez une constante : ensuite utilisez-la comme 12V / R",
  },
};
...
export default function CalculatorScreen() {
  ...
  // 検索欄がキーボードの裏に隠れる（実機で報告）。寸法の決め方は lib/sheet-layout.ts の純関数
  // （resolveSheetKeyboardLayout）に置いてあり、**定数の編集シートも同じものを通す**——画面ごとに
  // 書くと必ずどこかが旧いままになる。理由と過去に踏んだ穴はそちらのコメントにまとめてある。
  const [keyboardHeight, setKeyboardHeight] = useState(0);
  const sheetStyle = useMemo(
    () => [styles.compactSheet, resolveSheetKeyboardLayout(keyboardHeight, windowHeight, insets.bottom)],
  ...
  const { constants, history, favoriteUnits, upsertConstant, removeConstant, addHistoryEntry, clearHistory, isLoading: isHistoryLoading } = useCalculatorStore();
  const { isPro } = usePro();
  ...
  // 「定数」パネルからそのまま定数を足す・直す。**以前は定数の編集口がライブラリタブの中にしか
  // 無く**、「電卓で使うものなのにどこで編集するのか分からない」と報告された。定数の名前が
  // 並んでいるまさにこの場所が直したいと思う場所なので、鉛筆チップで編集モードへ切り替える。
  const [isEditingConstants, setEditingConstants] = useState(false);
  // 編集シート。開閉・編集対象・セッション番号を分けて持つのは NotebookEditorSheet と同じ形で、
  // **セッション番号をシートの `key` にすると、開くたびにフォームが作り直されて前回の入力が
  // 残らない**（effect で props を state へ写す形にしなくて済む）。閉じるときは対象を消さない
  // ——消すとスライドアウトの最中に空のフォームが滑り落ちる。
  ...
  const openConstantSheet = useCallback((symbol: string | null) => {
    setConstantSheetSymbol(symbol);
    setConstantSheetSession((current) => current + 1);
    setConstantSheetVisible(true);
  }, [setConstantSheetSymbol, setConstantSheetSession, setConstantSheetVisible]);
  const editingConstant = constantSheetSymbol ? constants.find((entry) => entry.symbol === constantSheetSymbol) : undefined;
  // **useMemo で包むこと。** ExpressionKeyboard は memo してあるので、毎レンダー新しい
  // オブジェクトを渡すとキーパッドのメモ化が丸ごと効かなくなる（打鍵ごとに20キーを作り直す）。
  const constantActions = useMemo(
    () => ({
      isEditing: isEditingConstants,
      onToggleEditing: () => setEditingConstants((current) => !current),
      onEdit: (symbol: string) => openConstantSheet(symbol),
      onCreate: () => openConstantSheet(null),
    }),
    // setState の関数は安定なので依存に入れても評価回数は変わらないが、**書かないと
    // React Compiler が「推論した依存と食い違う」としてこのコンポーネントの最適化を丸ごと
    // 飛ばす**（react-hooks/preserve-manual-memoization）。
    [isEditingConstants, openConstantSheet, setEditingConstants],
  );
  ...
  // 単位パレット（カテゴリ行＋候補レール）。**計算ノートと同じフック・同じ部品**を使う
  // （hooks/use-unit-rail.ts と components/ui/unit-rail.tsx）。候補の出し方・置き換える範囲の
  // 決め方は全部そちらにあるので、この画面で増やした絞り込みはノートにもそのまま効く。
  // 進数入力モード中は式が生の桁なので止める（FF のような桁を単位として解析させない）。
  const unitRail = useUnitRail({
    analysis,
    enabled: baseInputMode === null,
    expression,
    fixSelection,
    identifiers,
    prefixEntry,
    ...
    recentUnits,
    requiredGroup: requiredUnitGroup,
    selection,
    unitSystem,
  });
  const resetPalette = unitRail.reset;
  const activePrefix = unitRail.activePrefix;
  const paletteTarget = unitRail.target;

  const visibleDiagnosis = unitRail.hint.kind === "complete" ? "" : liveDiagnosis;
  ...
  const pressKey = (key: string, literal = false) => {
    ...
    // 定数の定義（`R = 4.7kΩ`）を書くためのキーで、押すと確定してしまっては使いようが無い
  ...
  // 単位シートは「結果をどの単位で表示するか」（display）と「式へ挿す単位を綴りで探す」（insert）の
  // 2用途。挿入側の通常の経路は単位パレット（カテゴリ行＋レール）で、シートはカテゴリを辿るより
  // 綴りで探した方が早いときの入口（「他 ›」の検索は表示単位にしか効かず、入力側から使えないと
  // 報告された）。
  const openUnitPicker = (mode: "display" | "insert" = "display") => {
    ...
    if (mode === "insert") setTimeout(() => unitSearchRef.current?.focus(), 350);
  };

  // シートのチップの点灯は「今の表示単位」を示すものなので、式へ挿す用途のときは何も点けない。
  const pickerChipActive = (symbol: string) => unitPickerMode === "display" && displayUnit === symbol;

  const chooseUnit = (unit: string) => {
    if (unitPickerMode === "insert") applyUnitCandidate(unit);
    else applyTargetUnit(unit);
    setUnitSearch("");
  ...
  useEffect(() => {
    ...
    setFixSelection(null);
    setError("");
  }, [isHistoryLoading, history, expression, resetPalette]);
  ...
  }, [copy, language, quick, resetPalette]);
  ...
  }, [copy, language, presetExpression, presetUnit, resetPalette]);
   ...
   *
   * 中身は計算ノートと共用の UnitRail（components/ui/unit-rail.tsx）。この画面が足すのは
   * 置き場所の余白と、右端の進数入力の入口だけ。
   *
  ...
  const renderUnitRail = (atBottom: boolean) => {
    ...
    return (
      <UnitRail
        bottomGap={atBottom ? layout.keyRowGap - 2 : undefined}
        language={language}
        onApply={applyUnitCandidate}
        onOpenSearch={() => openUnitPicker("insert")}
        state={unitRail}
        trailing={
  ...
  // **キーパッドは打鍵のたびに作り直さない。**
  // `pressKey` は式・キャレット・パレットの状態を読むので毎レンダー新しい関数になる。それを
  // そのまま `onPress` に渡すと20個のキーのpropsが毎回変わり、キーパッド全体が作り直される。
  // 実機（moto g52j 5G・デバッグビルド）のReact Profilerで、キーパッドを含む下のクラスタが
  // **1打鍵あたり108〜154ms**（画面全体の約7割）かかっていた。キーの見た目と挙動は
  // 進数入力モード以外では何も変わらないので、参照をrefで固定してJSXごとメモ化する。
  // **ref の更新は effect で行うこと**（レンダー中に書くと react-hooks/refs が誤検知する）。
  ...
  useEffect(() => {
    moveCaretRef.current = moveCaret;
    toggleKeyboardInputRef.current = toggleKeyboardInput;
  });
  const stableMoveCaret = useCallback((delta: 1 | -1) => moveCaretRef.current(delta), []);
  const stableToggleKeyboardInput = useCallback(() => toggleKeyboardInputRef.current(), []);
  const applyTargetUnitRef = useRef(applyTargetUnit);
  useEffect(() => {
  ...
  const conversionChipRail = useMemo(
    () => (
        ...
        <Pressable accessibilityLabel={copy.noUnit} onPress={() => stableApplyTargetUnit("")} style={({ pressed }) => [styles.convertChip, siChipActive && styles.convertChipActive, pressed && styles.pressed]}>
          <Text style={[styles.convertChipText, siChipActive && styles.convertChipTextActive]}>SI</Text>
        </Pressable>
        {conversionUnits.map((symbol) => (
          <Pressable accessibilityLabel={symbol} key={symbol} onPress={() => stableApplyTargetUnit(symbol)} style={({ pressed }) => [styles.convertChip, displayUnit === symbol && !isFallbackUnit && styles.convertChipActive, pressed && styles.pressed]}>
            <Text style={[styles.convertChipText, displayUnit === symbol && !isFallbackUnit && styles.convertChipTextActive]}>{symbol}</Text>
          </Pressable>
        ))}
      </ScrollView>
    ),
  ...
  return (
    <ScreenContainer className="px-4" containerClassName="bg-background">
      <View style={styles.screen}>
        ...
        <View style={styles.inputCard}>
          {/* 色分けしたトークン列そのものが入力欄。かつては TextInput の下に同じ式のプレビュー行を
              ...
              描かないので「次に何がどこへ入るか」はプレビュー行にしか出ていなかった。1枚にまとめ、
              キャレットはフォーカスの有無に関わらずこちらで描く。 */}
          <View style={styles.inputRow}>
            <Pressable
              // 文字の無い余白をタップしたときは末尾へ。一片の上をタップしたときは
              // ExpressionPiece 側が先に応答するので、ここには来ない。
              onPress={() => placeCaretFromTap(expression.length)}
            ...
            {/* OSのキーボードの受け口。キーパッドには英字が無く、定数名（R = 4.7kΩ）や単位名は
            ...
            <TextInput
              ...
              style={styles.hiddenExpressionInput}
            />
            {/* **`=` ではなくアイコンにしてある**（2026-09-22。利用者からの指摘）。結果は打つそばから
                出ているので、このボタンが実際にやるのは**確定＝履歴に残す・定数定義を保存する・
                エラーを出す**の3つだけ。`=` という字面は「押すと計算される」と読めてしまい、実際の
                役割と食い違っていた。保存のアイコン（`square.and.arrow.down`）にして、読み上げと
                撮影スクリプトが引くラベルも `copy.saveCalculation` に変えてある。 */}
            <Pressable accessibilityLabel={copy.saveCalculation} onPress={() => submitCalculation()} style={({ pressed }) => [styles.calculateButton, pressed && styles.pressed]}>
              <IconSymbol name="square.and.arrow.down" size={22} color={colors.onPrimary} />
            </Pressable>
          </View>
              ...
              {unitSearch.trim() ? (
                <>
                  ...
                  {searchSuggestions.length ? (
                    <View style={styles.chips}>{searchSuggestions.map((suggestion) => renderUnitChip(suggestion, () => chooseUnit(suggestion.unit.symbol), pickerChipActive(suggestion.unit.symbol)))}</View>
                  ) : (
      ...
      {/* グローバル定数の編集シート。**ライブラリタブと同じ部品**（フォームを2つ持つと、
          片方にだけ入る改良が必ず生まれる）。削除はシートの中から。

          **一度も開いていない間はマウントしない。** 閉じている Modal でも JSX の中身は親の
          レンダーで評価されるので、置きっぱなしにすると式を1文字打つたびにシートの単位レールと
          キーパッドの要素まで作り直すことになる（閉じた Modal の中身を useMemo で括っているのと
          同じ事情）。一度開いたあとは残すので、閉じるときのスライドアウトはそのまま出る。 */}
...
const createStyles = (colors: ThemeColorPalette, layout: CalculatorLayout) => StyleSheet.create(scaleFontSizes({
  ...
  // 「表記の分類」であることを読ませる（tokenIdentifierと同じくwarning系を分類の色として使う）。
