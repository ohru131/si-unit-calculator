...
export const ENG_POWER_SEEDS: NotebookSeed[] = [
  ...
  },
  {
    ...
    // **このノートだけは呼び寸法に `exact` を残す。** J の式が `D^4-d^4` という**減算**を通るので、
    // 測定値どうしの加減算として桁が読めなくなり（lib/significant-figures.ts）、印を外すと
    // ねじり応力が `29.10261817 MPa` の生値に戻る。他のノートは印をやめて桁を書き分けてある。
    localConstants: [
...
export const ENG_ELEMENTS_SEEDS: NotebookSeed[] = [
  ...
  },
  {
    ...
    localConstants: [
      { symbol: "T", expression: "250N*m" },
      { symbol: "d", expression: "30.0mm" },
      { symbol: "b", expression: "8.00mm" },
      { symbol: "L_k", expression: "40.0mm" },
    ],
    steps: [
