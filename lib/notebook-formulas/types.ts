...
export type NotebookSeedConstant = {
  ...
  /**
   * **この数は測定値ではない**（有効数字に数えない）。図面の呼び寸法（板幅60mm・穴径Ø20mm・
   * 板厚8mm）、個数・回数（巻数・本数）、規格や定義で決まる値に付ける。
   *
   * 付けないと2つの形で結果が壊れる: (1) `t=8mm` が1桁の測定値として数えられ、結果が
   * 1桁に落ちて丸めごと止まる、(2) `(w-d)` のような厳密値どうしの引き算が「加減算が混ざる式」
   * として桁の読み取りを塞ぐ。どちらも「測っていない数から精度の話は生まれない」で説明できる。
   *
