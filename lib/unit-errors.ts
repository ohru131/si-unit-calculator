...
const UNIT_ERROR_MESSAGES: Record<AppLanguage, UnitErrorMessageCatalog> = {
  en: {
    ...
    invalidConstantDefinitionFormat: () => `Define a constant in the form "R = 4.7kΩ".`,
    constantSymbolIsUnit: ({ symbol }) => `"${symbol}" is already a unit symbol. Name the constant something else, such as ${symbol}1.`,
  ...
  ja: {
    ...
    invalidConstantDefinitionFormat: () => "定数は「R = 4.7kΩ」の形式で定義してください。",
    constantSymbolIsUnit: ({ symbol }) => `「${symbol}」は単位の記号です。定数は ${symbol}1 のように別の名前にしてください。`,
  ...
  es: {
    ...
    invalidConstantDefinitionFormat: () => `Define una constante con el formato "R = 4.7kΩ".`,
    constantSymbolIsUnit: ({ symbol }) => `«${symbol}» ya es un símbolo de unidad. Ponle otro nombre a la constante, por ejemplo ${symbol}1.`,
  ...
  "pt-BR": {
    ...
    invalidConstantDefinitionFormat: () => `Defina uma constante no formato "R = 4.7kΩ".`,
    constantSymbolIsUnit: ({ symbol }) => `"${symbol}" já é um símbolo de unidade. Dê outro nome à constante, por exemplo ${symbol}1.`,
  ...
  de: {
    ...
    invalidConstantDefinitionFormat: () => `Definiere eine Konstante im Format „R = 4.7kΩ“.`,
    constantSymbolIsUnit: ({ symbol }) => `„${symbol}“ ist bereits ein Einheitenzeichen. Gib der Konstante einen anderen Namen, zum Beispiel ${symbol}1.`,
  ...
  fr: {
    ...
    invalidConstantDefinitionFormat: () => `Définissez une constante au format « R = 4.7kΩ ».`,
    constantSymbolIsUnit: ({ symbol }) => `« ${symbol} » est déjà un symbole d’unité. Donnez un autre nom à la constante, par exemple ${symbol}1.`,
