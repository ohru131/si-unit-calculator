...
const EN_COPY = {
  ...
  theme: "Appearance",
  // 折りたたみが閉じた行の右端に出す短縮形。themeSystem/standardUS/standardJISは展開した
  // ラジオの選択肢として読ませる説明文（例「日本のJIS規格（カップ = 200mL）」）なので、
  // 行の値の位置に置くと狭い端末幅で肝心の数値ごと切れて読めなくなる。
  ...
  adsTitle: "Ads",
  adsHint: "The free version shows a small banner ad. Upgrade to Pro to remove it.",
  adsFreeActive: "Ads are hidden.",
  adsUpgrade: "See Pro plans",
  region: "Region",
  // ポリシー本文は英語のみで用意している（法的文書の誤訳リスクを避けるため）。
  // このラベルだけは他のUI文言と同じくアプリの言語に従う。
  ...
  // 折りたたみが閉じている行の右端に出す件数表示（設定画面のSettingsSectionのvalue prop）。
  customUnitCount: "{count} custom units",
  ...
  // 破壊的操作なので、プリセットのリセットと同じくこの設定画面に集約する。設定画面では折りたたみ
  // カードの見出しとしても使うので、単独で読んでも意味が通る文言にする。
  backupSectionTitle: "Backup & restore",
  backupNotebooksTitle: "Notebooks",
  // 書き出されるのは自作ノート＋プリセットへの編集内容だけで、プリセット本体は含まれない
  // （lib/notebooks-backup.ts の createNotebooksBackup を参照）。「すべて書き出す」は実態と
  // 違うので、キー名・文言ともに「自分のノート」に揃える。
  backupNotebooksExportOwn: "Export your notebooks",
  backupNotebooksExportOwnHint: "Exports the notebooks you created yourself, plus any edits you made to preset notebooks. Preset notebooks themselves are not included.",
...
const COPY: Record<AppLanguage, Record<TranslationKey, string>> = {
  ...
  ja: {
    ...
    adsTitle: "広告",
    adsHint: "フリー版には小さなバナー広告が表示されます。Proへのアップグレードで非表示にできます。",
    ...
    backupNotebooksExportOwn: "自分のノートを書き出す",
    backupNotebooksExportOwnHint: "自分で作成した計算ノートと、プリセットに加えた編集内容を書き出します。プリセット本体は含まれません。",
  ...
  es: {
    ...
    adsTitle: "Anuncios",
    adsHint: "La versión gratuita muestra un pequeño anuncio en banner. Actualiza a Pro para quitarlo.",
    ...
    customUnitCount: "{count} unidades personalizadas",
    ...
    backupNotebooksExportOwn: "Exportar tus cuadernos",
    backupNotebooksExportOwnHint: "Exporta los cuadernos que creaste tú mismo, además de las ediciones que hiciste en los cuadernos preestablecidos. Los cuadernos preestablecidos en sí no se incluyen.",
  ...
  "pt-BR": {
    ...
    adsTitle: "Anúncios",
    adsHint: "A versão gratuita exibe um pequeno banner de anúncio. Faça upgrade para o Pro para removê-lo.",
    ...
    backupNotebooksExportOwn: "Exportar seus cadernos",
    backupNotebooksExportOwnHint: "Exporta os cadernos que você mesmo criou, além das edições feitas em cadernos predefinidos. Os cadernos predefinidos em si não são incluídos.",
  ...
  de: {
    ...
    adsTitle: "Werbung",
    adsHint: "Die kostenlose Version zeigt ein kleines Werbebanner. Upgrade auf Pro, um es zu entfernen.",
    ...
    customUnitCount: "{count} eigene Einheiten",
    ...
    cancel: "Abbrechen",
    backupSectionTitle: "Sicherung und Wiederherstellung",
    backupNotebooksTitle: "Rechenhefte",
    backupNotebooksExportOwn: "Eigene Rechenhefte exportieren",
    backupNotebooksExportOwnHint: "Exportiert die Rechenhefte, die du selbst erstellt hast, sowie alle Bearbeitungen an vordefinierten Rechenheften. Die vordefinierten Rechenhefte selbst sind nicht enthalten.",
  ...
  fr: {
    ...
    adsTitle: "Publicités",
    adsHint: "La version gratuite affiche une petite bannière publicitaire. Passe à Pro pour la retirer.",
    ...
    customUnitCount: "{count} unités personnalisées",
    ...
    backupNotebooksExportOwn: "Exporter mes carnets",
    backupNotebooksExportOwnHint: "Exporte les carnets que tu as créés toi-même, ainsi que les modifications apportées aux carnets prédéfinis. Les carnets prédéfinis eux-mêmes ne sont pas inclus.",
