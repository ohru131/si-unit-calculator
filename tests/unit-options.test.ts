...
describe("compatibleUnitOptions", () => {
  ...
  it("名前の付いたグループが無い合成次元（クーロンの法則のk＝N・m²/C²）でもフォールバックで0件にならない", () => {
    ...
    // 数値直後の単位サフィックスは評価器と同じく丸ごと1つの複合単位として読むので、
    // 候補もその表記1件になる。次元の違う因子（N単体など）まで並べると、押しても変換できず
    // エラーになるだけなので出さない。ここでの目的はレールを空にしないこと。
    expect(options.map((option) => option.symbol)).toEqual(["N*m^2/C^2"]);
  });

  it("同じ形の万有引力定数G（N・m²/kg²）でも候補が0件にならない", () => {
    ...
    const options = compatibleUnitOptions(quantity, "metric", { expression });
    expect(options.map((option) => option.symbol)).toEqual(["N*m^2/kg^2"]);
  });
  ...
  it("運動量（kg*m/s）のように次元に対応するグループが無い手順結果でも、表示単位からフォールバックできる", () => {
    ...
    expect(options.map((option) => option.symbol)).toEqual(["kg*m/s"]);
  });
