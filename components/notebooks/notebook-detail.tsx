                ...
                <View key={result.step.id} style={[styles.resultCard, isFinalStep && result.quantity ? styles.resultCardFinal : null]}>
                  <TextInput
                    ...
                    onChangeText={(text) => {
                      ...
                      // 名前があるときも、以前は問答無用でtitleを記号名(name)に置き換えていたが、それだと
                      // プリセットの翻訳済み表示タイトル（例:「速さ v」）を名前欄に触れただけで記号名だけに
                      // 潰してしまっていた。titleが「以前この仕組みで記号から自動生成されたもの」
                      // （空、または直前のresultSymbolと同じ）のときだけ追従させ、人間が書いた/
                      // プリセットのタイトルはそのまま保つ。
                      const nextTitlePatch = isAutoGeneratedStepTitle(result.step.title, result.step.resultSymbol) ? { title: name } : {};
                      updateStepField(
                        result.step.id,
                        name ? { resultSymbol: name, expression: value, ...nextTitlePatch } : { resultSymbol: undefined, expression: value },
                      );
