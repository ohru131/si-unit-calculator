...
- OSのキーボードは編集キー行のキーボードキーを押したときだけ出すようにした（式をタップしても出ない。ネイティブではキーボードがキーパッドを覆い、式を組み立てるたびに閉じる操作が要っていたため。Webは物理キーボードのために従来どおり）。
- 接頭語キーを押した直後の候補を、今の式に出ている単位と直近に使った単位に合わせて並べ替えるようにした（`12V / 4.7k` なら kV・kΩ・kW が km・kg より先に出る）。
