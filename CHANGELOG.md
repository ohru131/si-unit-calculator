...
### 変更
- 電卓の入力欄の文字を少し大きくした（19→21px）。
- `1/5` のように自分で割り算を打ったときは、結果カードに分数表示（分数・π）のチップを出すようにした。これまでは有限小数になる分数を一律で隠していたため、`1/5` と打っても `0.2` しか出せなかった。

### 修正
- キーパッドのキーの当たり判定を、見た目のキーからセル全体（48dp）へ広げた。これまでキーとキーの間に6dpの無反応な帯があり、右下の `=` は当たり判定が42dpしかなかったため、狙いが少し下へ逸れると真下のタブバー（設定）に当たって画面が切り替わっていた。
- キーパッドとタブバーの間の余白を広げた（画面の高さに余裕がある端末のみ 4→12dp。実測のすき間は約1mm→約3.6mm）。
- 分数表示のとき、分子の数字の上が切れる問題を修正した（Android・iOS）。
- 下から出るシート（数学・サンプル・履歴・単位ピッカー）の最下段が、Androidの3ボタンナビゲーションバーに隠れる問題を修正した（数学シートの π・e が半分隠れていた）。
