...
| iOS商品 | 非消費型（Non-Consumable）の買い切り商品 | App Store向けPro購入 |
| Android商品 | Google Playの1回購入商品（非消費型） | Google Play向けPro購入 |
| Paywall | `default` offeringに接続 | 取得失敗時のフォールバック用（通常はアプリ内の購入カードから直接購入する） |

価格はApp Store・Google Playの地域別価格と競合調査を踏まえ、RevenueCatダッシュボードで最終決定する。アプリコードには価格を固定せず、offeringから取得したストアのローカライズ済み価格文字列をそのまま購入ボタンに出すため、ダッシュボード側の変更がそのまま反映される。

## ブラッシュアップ済みの購入体験

Proタブでは、Proの便益、購入画面、購入復元、ストア版でのみ実決済できる旨を明示する。電卓タブでは、CSV操作をPro導線に接続する。Pro利用者にはマイ単位セットを追加表示し、保存済みの単位を式へ挿入できる。
...
5. Shipaton審査用に審査員向けプロモコード（iOS: プロモコード、Android: プロモーションコード）を準備する。買い切り商品に無料トライアルは設定できない。
