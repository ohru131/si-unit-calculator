...
Define your own units (shaku = 0.303m). Save constants like W1 = 3cm and use them directly in any expression.
...
自分の単位と定数
独自の単位も登録できます（shaku = 0.303m）。W1 = 3cm のように任意の定数も保存でき、式の中でそのまま使えます。
...
Define tus propias unidades (shaku = 0.303m). Guarda constantes como W1 = 3cm y reutilízalas directamente en cualquier expresión.
...
Defina suas próprias unidades (shaku = 0.303m). Salve constantes como W1 = 3cm e use de novo em qualquer expressão.
...
Lege eigene Einheiten an (shaku = 0.303m). Speichere Konstanten wie W1 = 3cm und nutze sie direkt in jedem Ausdruck.
...
Définissez vos propres unités (shaku = 0.303m). Enregistrez des constantes comme W1 = 3cm et réutilisez-les directement dans n'importe quelle expression.
