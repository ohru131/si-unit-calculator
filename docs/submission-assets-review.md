...
| アセット | 所見 | 今回の判断 |
|---|---|---|
| ストアアイコン（青い電卓の本体・液晶に `1kΩ × 1mA`・白と淡青の立体キートップ） | 端末フレームなし。液晶の `1kΩ × 1mA` は意匠の一部（宣伝文句・価格・「新着」のような販促テキストではない）で、電卓本体は**円形マスクで四隅が削られても欠けない安全域の内側**に収めてある。**`submission-assets/store/play-store-icon-512.png` として作成済み**（512×512px・不透明であることをChromiumで実測確認済み） | **512×512px, 32-bit PNGへ書き出す。** 1024×1024pxからのリサイズではなく、512×512pxのマスターを新規に用意し、安全余白（内側15〜18%目安）を守ること（`docs/android-submission-checklist.md`2番参照）。作成後は実際の書き出しファイルの寸法・ビット深度・安全余白を確認すること |
...
| 既存素材一式（PR #41時点、コミット`12584d2`） | **厳密値表示（分数・π・√、PR #42）もカテゴリグリッド194件（PR #46）もノート検索（PR #48）も写っていない。** PR #41以降にmainへ入った変更が撮影対象からごっそり抜けている | **撮り直し必要。** 下記「今回追加で必要になった撮影対象」と`docs/screenshot-capture-plan.md`のカット4a・5aを新規撮影する |

## 今回追加で必要になった撮影対象（旧版に存在しなかった機能）

いずれも旧版のレビュー対象に無かった機能。裏取りは`CLAUDE.md`「直近の作業履歴」の該当番号と実装ファイル。

- **計算ノートのカテゴリグリッド（幅を見せる）** — 194件・38カテゴリ（最上位9枚＋親子2階層）。裏取り: 直近の作業履歴3・6・20番、`lib/notebook-formulas/source/categories.ts`実行結果
...
- **計算ノートの検索** — ライブラリタブ上部の検索欄でタイトル・説明文・カテゴリ名を横断絞り込み。裏取り: `lib/notebook-search.ts`、PR #48。194件・38カテゴリになったことで「探す」導線自体が訴求点になる
...
Google Playは**1024×500pxのフィーチャーグラフィック**をストア掲載の必須アセットとして要求する（App Storeには相当する項目が無いため、旧版では完全に抜けていた）。**`submission-assets/store/play-feature-graphic-<lang>-1024x500.png` として6言語ぶん作成済み**（`scripts/generate-feature-graphic.mjs` で再生成できる。以前の英語1枚きりの `play-feature-graphic-1024x500.png` は、生成スクリプトが残っておらず旧アイコン・旧件数のまま凍結されていたので置き換えた）（1024×500px・不透明であることをChromiumで実測確認済み）。既存アイコンの意匠と、そこから抽出した実際の地色（#03568a）を使った横長レイアウトで、要素は中央712px以内の安全域に収めてある（端は機種によって切られるため）。**アルファチャンネル不可**（透過PNGは使えない）。作成後は実際のファイルの寸法・形式（JPEGまたは24-bit PNG、アルファチャンネル無し）を確認すること。
