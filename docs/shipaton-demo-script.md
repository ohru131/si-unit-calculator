...
| 0:07–0:15 | Press `AC`, type `1kΩ × 1mA`; the result card shows `1 V` without pressing `=` (the same formula as the app icon, the feature graphic and the short description) | **1kΩ × 1mA is exactly 1 V — the prefixes cancel themselves.** |
| 0:15–0:22 | Confirm `1kΩ × 1mA` with `=` (so it lands in the history used at 1:38), press `AC`, type `5m + 1kg` and press `=`; the dimension-mismatch error appears in place of a result | **Mix incompatible units, and it tells you — instead of a wrong answer.** |
