...
- **The recording in `submission-assets/demo/unit-calculator-demo-en-silent.webm` follows this script**: 1:53, 1080×1800, 25 fps, no audio track, English captions burned in from `demo-captions-en.srt`. Re-record it with `node scripts/record-demo-video.mjs` whenever the shot list or the captions change (the previous take — 2:30, 540×900, 112 notebooks — has been replaced). **Re-record it whenever the calculator screen changes, too**: the take from 2026-09-15 is the first one that shows the rebuilt expression field (the colour-coded token row *is* the input now, with an always-visible app-drawn caret; the old "?" help button and the separate preview row below the field are gone). Captions and shot list did not change for that take. The take from 2026-09-18 added the moved unit rail: the candidate rail sits below the prefix-key row instead of under the expression field, and its leading two-line chip (category name + action label, with a chevron) opens the unit-category row on tap — the always-visible category row is gone. The take from 2026-09-20 shows the shared expression keyboard: the prefix-key row and the unit rail are now the contents of the `Unit` panel, and a **tool row** (`Const` / `xⁿ` / `f(x)` / `αβ` / `ABC` / `Unit` / keyboard) sits between that panel and the keypad, so the rail is no longer the row directly above the keypad. `=` also left the keypad (the answer appears as you type; the `=` next to the expression field is the only one left, apart from the constant-definition `=` inside the `ABC` panel). The take from 2026-09-22 shows the commit button as an **icon instead of `=`**: the answer already appears as you type, so that button only saves the calculation (history, and the constant definition when you typed one), and it now carries a save icon with the accessibility label `Save calculation`. Captions and shot list did not change for any of these takes; only the `0x` pill's location in the 0:50 row and the `=` wording were reworded.
...
| 0:07–0:15 | Press `AC`, type `1kΩ × 1mA`; the result card shows `1 V` without touching the save button (the same formula as the app icon, the feature graphic and the short description) | **1kΩ × 1mA is exactly 1 V — the prefixes cancel themselves.** |
| 0:15–0:22 | Confirm `1kΩ × 1mA` with the save button (so it lands in the history used at 1:38), press `AC`, type `5m + 1kg` and press save; the dimension-mismatch error appears in place of a result | **Mix incompatible units, and it tells you — instead of a wrong answer.** |
...
| 1:38–1:45 | Back on the UnitCalc tab: open the saved-history sheet and scroll it, showing the entries saved during the earlier scenes | **Full calculation history — free, for everyone, unlimited.** |
...
- **The commit button is to the right of the expression field, and it is no longer labelled `=`
  (2026-09-20, re-labelled 2026-09-22).** The shared expression keyboard dropped `=` from the keypad —
  the result appears as you type, so that button is only the commit step (save to history, save a
  constant definition, raise the error). It now shows a save icon; `scripts/record-demo-video.mjs`
  drives it through `submit(page)` (`getByLabel("Save calculation")`). The earlier `key(page, "=")`
  and `getByLabel("Result")` both stop working, because `Result` is the result card's heading and the
  only remaining `=` key lives inside the `ABC` panel for constant definitions (`R = 4.7kΩ`).

- **The dimension-mismatch error only appears after the commit.** The real-time preview simply leaves the result blank for `5m + 1kg`; the red error is part of the confirm step (`app/(tabs)/index.tsx` — the save button is what commits, saves and raises errors). Scene 3 therefore presses it.
- **The history sheet is only populated by the commit.** Scene 2 deliberately shows the result *without* it, so the confirm for that expression happens at the head of scene 3, and scenes 4–7 each confirm at their end. That is what fills the sheet shown at 1:38.
...
- **Added the notebook search scene (1:04–1:12, PR #48).** With 194 notebooks across a two-level hierarchy, "how do I find one" is the obvious question the old shot list left unanswered. `photography` is used because it returns exactly the 7 Photography notebooks purely by matching the *category* name, which demonstrates the feature in one shot.
- **Updated the notebook counts and category names (PR #46)**: 112 → **194** notebooks, and the old nine names (Science / High-school physics / Electricity basics / Astronomy / Fitness / Chemistry / Cars & bikes / Cooking / Materials) → the current nine top-level cards.
