...
3. **ドメイン別プリセット公式集184件を最初から搭載**（2026-09-05時点。CLAUDE.md「直近の作業履歴」20番） — ノート型競合（Soulver / CalcNote / mathote）はいずれも**白紙から自分で書く前提**でプリセットを持たない
...
> **【2026-09-03 追記】この節の提案1〜4は実装済み。** サブスクは廃止して買い切り1本にし、
> 履歴5件制限も撤廃した（Proの特典は広告非表示・CSVエクスポート・マイ単位セットの3点）。
> 以下の「現状」と価格の3択案は**調査時点の記録**であって現在の仕様ではないので、
> 仕様を確認するときは `docs/shipaton-general-product-plan.md` を見ること。
...
4. カテゴリグリッド（184件・38カテゴリの「幅」を見せる）
5. iOSウィジェット／6言語対応
...
- 「革新的なアイデア／美しいUI／丁寧なインタラクション・アニメーション」が評価軸であり、KaTeXでの本物の数式描画、7次元次元検証、複合単位のカーソル位置認識置換、184件のプリセット、6言語完全ローカライズは「電卓という地味なカテゴリで異例に作り込まれている」と言える
