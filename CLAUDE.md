...
- `app/(tabs)/index.tsx` の `EDIT_KEYS` / `moveCaret` — **キーパッド直上の編集キー行**（`<` `>` `x²` `x³` `xʸ` `×10ⁿ`）。**入力欄をタップするとOSのキーボードが上がってキーパッドがほぼ隠れる**ので、入力欄を触らずに式を組み立てられる範囲を広げるためのもの。`^` は数学シートにしか無く、しかも上級モード限定だった。
  ...
  - **行を増やせるのは1行ぶんだけ。** 360×640 の端末ではキーパッド下段（`. 0 ⌫ =`）が元から際どく、2行足すとタブバーに潜って `=` が押せなくなる。数学ボタンを単独の行から編集キー行へ移し（`keypadTools` は廃止）、行の高さと余白も詰めて取り戻してある。**行を足すときは 360×640 でキーパッド下段の `=` の下端とタブバーの位置を実測すること**（画面の高さと比べるだけでは足りない。タブバーが下端70pxほどを占める）。
