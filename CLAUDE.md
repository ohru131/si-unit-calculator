...
- `lib/expression-caret.ts` の `buildCaretPreview` — **入力欄の下のプレビュー行にキャレット（カーソル）と選択範囲を描くための純関数**。セグメント列をキャレット位置（範囲選択なら両端）で切り分けて返す。
  - **TextInput はフォーカスが当たっていない間キャレットを描かない。** この電卓は「入力欄を触らずキーパッドで式を組み立てる」設計（入力欄をタップするとOSのキーボードが上がってキーパッドがほぼ隠れる）なので、`< >` の編集キーを押しても**カーソル位置が画面のどこにも出ていなかった**。プレビュー行を唯一のキャレット表示の場所にしてある。
  ...
  - **`showSoftInputOnFocus={false}` は使っていない。** iOSのFabric実装（`RCTTextInputComponentView.mm` の `_setShowSoftInputOnFocus`）には `// Hides keyboard, but keeps blinking cursor.` とあり、**iOS/Androidとも「キーボードを出さずに本物のキャレットだけ出す」ことは実際に可能**。それでも採らなかったのは、**キーパッドに英字が無く、定数名（`W = 3cm`）や単位名の打ち込みにOSのキーボードが要る**ため。恒久的に塞ぐと文字入力ができなくなる。「キーパッド操作中だけ塞ぐ」形にするなら、`showSoftInputOnFocus` を実機で切り替えられるか（Androidは再フォーカスが要る可能性）の検証が先。**この環境ではWebしか確認できないので、やるなら実機で確かめること。**
  - **Playwrightで検証するときは `fill()` を使わないこと。** `fill()` は値だけ入れて選択範囲のイベントを飛ばさないので `selection` が初期値のまま残り、「キャレットが動かない」という**実装ではなくテストの誤り**を踏む（実際に踏んだ）。`pressSequentially()` で実際のキー入力にする。
