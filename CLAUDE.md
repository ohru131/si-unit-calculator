...
- `lib/sample-calculations.ts` + 電卓のサンプルシート（`app/(tabs)/index.tsx` の `samplesSheet`） — 式を丸ごと差し替える見本。
  ...
  - **カテゴリのチップ行は一覧の「下」に折り返しで置く。** 下に置くのは、シートの高さが件数で変わるとチップの位置が上下するため（「位置が変わってはいけないものは画面の下端から積む」）。横スクロールにしないのは9件あって端のタブが存在ごと気付かれないため。独語は3行になるので、この行のチップだけ単位ピッカーより一回り小さくしてある（`sampleCategoryChip`）。一覧は `flexShrink: 1`・チップ行は `flexShrink: 0` を明示する（RNの既定は 0 なので、書かないとチップが潰れる）。
