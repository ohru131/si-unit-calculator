...
- **チップ列が「表示モードの切り替え」と「入力モードの開始」を兼ねている。** 式が空なら入力モードの開始、式があって結果が対象なら表示基数の切り替え。**この2つは式が空かどうかで排他的に決まる**ので同じチップ列を使い回せている（選択中の基数も `baseInputMode ?? activeBase` の1値で表せる）。この排他性が崩れる変更をするなら、チップ列を分けること。
