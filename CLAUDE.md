...
### 現在の基準値（2026-09-22時点、ノート編集シートをアプリ内キーパッドにし、呼び寸法を桁で書き分けた後）

- `npx tsc --noEmit` → **`app/_layout.tsx` の `@/global.css` で1件のみ**（従来どおりの環境依存）。
- `npx vitest run` → **1180 passed / 2 failed**。失敗2件は従来どおり `tests/revenuecat.credentials.test.ts`（環境依存）。追加は `tests/notebook-keypad.test.ts` の結合フィールド6件。
- `npx expo lint` → **2エラー・0警告**（`app/(tabs)/index.tsx` の既存分のまま）。
- `pnpm notebook:rounding` → 378手順。`exact` が残っているシードの定数は **26件**（個数・回数・定義値＋加減算を通る2ノートの呼び寸法）。
- `npx expo export --platform web` が通る。Playwright（540×900・ja）で確認済み: 引張材ノートが `125 MPa` / `1.25 mm`（`≈` なし）で出る／編集シートで手順の欄をタップするとシート下端にキーパッドが出て、欄名（`軸応力 σ`）と「閉じる」が上段に付く／数字キー・⌫・接頭語キー・単位チップ（`k` → `km`）が効く／`定数` パネルにそのノートの記号（π・e・F・A・L・E）が並ぶ／`◀` で名前側へ戻ってから `αβ` パネルのギリシャ文字を挿すと**名前側に入る**。
- **未検証（この環境では Web しか動かせない）**: 編集シートの欄をタップしたとき OS のキーボードが出ないこと・⌨ キーで出ること（`showSoftInputOnFocus`）、キーパッドが開いたときのスクロール追従。**Android 実機で確認すること。**
