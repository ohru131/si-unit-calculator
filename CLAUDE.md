...
### 現在の基準値（2026-09-04時点、mainのコミット `c0152b3`＝PR #34マージ後）

上と同じ理由で古くなるが、**新しい失敗が自分の変更のせいかを切り分けるための基準**として置いておく。着手前に一度実行して、この数字と一致することを確かめてから作業を始めるとよい。

- `npx tsc --noEmit` → **エラー0**
- `npx vitest run` → **512 passed / 2 failed / 1 skipped**。失敗2件は上記の `tests/revenuecat.credentials.test.ts`（環境依存）。**それ以外が落ちたら自分の変更のせい。**
