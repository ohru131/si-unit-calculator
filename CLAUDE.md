...
- `components/notebooks/notebook-keypad.tsx` + `lib/notebook-keypad.ts` — **計算ノート詳細の値欄に付くアプリ内キーパッド**（2026-09-18。2026-09-19 からは中身が電卓と共用の `ExpressionKeyboard` になり、このファイルは上段（欄名・閉じる）と記号・単位チップの列だけを足すラッパー）。きっかけは「計算ノートでキーボードが出ず入力できない」という実機報告。値欄の入力手段が OS のキーボードだけだったため、それが上がらない端末（電卓の隠し TextInput と同じ現象。**原因は依然として未特定**）では値を一切変えられなかった。
  ...
  - **記号（定数・先行手順）と単位のチップもキーパッドの中の1行に集約した**（`symbols` / `units` props。以前は各欄の直下に「定数」レールと単位チップ列を欄ごとに出していた）。中身は編集シートのレールと同じ関数（`getLocalConstantFieldSuggestions` / `getStepFieldSuggestions` / `compatibleUnitOptions`）で求め、`activeField` にまとめて渡す。記号は下地付き・単位は枠付きで見分け、間に仕切りを置く。**結果カードの SI/kg… チップは表示単位の選択なのでキーパッドへは移していない**（入力ではない）。OS のキーボードを出している間もこの行は残す（記号・単位はキーボードでは打ちにくい）。
