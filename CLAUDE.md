...
- `lib/significant-figures.ts` — 結果を **`a × 10ⁿ` の科学表記**で出し、**入力式から読み取った有効数字で丸める**純関数（`inferSignificantDigits` / `toScientificNotation`）。`exact-value.ts` と同じく**表示のときだけ**の推定で、値に不確かさを持たせる方向へは進めない。
  ...
  - **「丸めたか」（`≈` と元の値の併記）も、数値ではなく表示される値どうしで比べる。** `rounded !== value` にすると**倍精度の誤差だけで真になる**——`40g ÷ 50cm³` は 0.8000000000000002 に落ちるので、2桁の `0.80` に対して「元の値 0.8」という**何も失われていない併記**が出ていた（プリセット365手順のうち1件で実際に出た。2026-09-21に修正）。小数表示は有効10桁（`MAX_MANTISSA_DIGITS`）で頭打ちなので、`Number(value.toPrecision(MAX_MANTISSA_DIGITS))` と突き合わせれば「画面で読める範囲で変わったか」が分かる。下の「何も得られない」判定（文字列が1文字も変わらない）とは別の穴なので、**両方要る**。
