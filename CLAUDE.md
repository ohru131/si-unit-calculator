...
- `components/notebooks/notebook-keypad.tsx` + `lib/notebook-keypad.ts` — **計算ノート詳細の値欄に付くアプリ内キーパッド**（2026-09-18。2026-09-19 からは中身が電卓と共用の `ExpressionKeyboard` になり、このファイルは上段（欄名・閉じる）と記号・単位チップの列だけを足すラッパー）。きっかけは「計算ノートでキーボードが出ず入力できない」という実機報告。値欄の入力手段が OS のキーボードだけだったため、それが上がらない端末（電卓の隠し TextInput と同じ現象。**原因は依然として未特定**）では値を一切変えられなかった。
  ...
  - **キーを押しても欄のフォーカスを外さないこと**（2026-09-25。Web で「`◀ ▶` が効かない・カーソルが見えない」と報告された）。フォーカスが外れた欄は `selection` prop で位置を動かしてもキャレットを描かないので、内部の位置は動いているのに画面では何も起きていないように見える。Web はキー（Pressable）が mousedown でフォーカスを奪うので、`NotebookKeypad` の root で `onMouseDown` の既定動作を止めている（RNW の View は `onMouseDown` を DOM へ渡す）。詳細画面と編集シートの両方に効く。**Web だけの問題**で、Android 実機では元からフォーカスが残りキャレットも動いていた（利用者が確認）ので、ネイティブ側には手を入れていない。
