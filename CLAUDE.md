...
- `lib/calculator-layout.ts` — **端末の文字サイズ・表示サイズの設定に電卓画面を追従させるための純関数**（`resolveCalculatorLayout` / `scaleFontSizes`）。電卓だけは「1画面に収める・縦スクロールさせない」構成なので、文字が大きい端末では**キーパッドがタブバーの下へ押し出されて数字キーが押せなくなる**（実機報告。知人のAndroidで最下段しか見えていなかった）。
  ...
  - **バナー広告のぶんを引いてから段階を選ぶ**（`bannerHeight`。2026-09-23）。`CalculatorBannerAd` は**Web版では描かれない**（`Platform.OS === "ios" || "android"`）ので、**このファイルに載っている Playwright の実測値はすべて広告の50dpを含んでいない**。引かないと、同じ640dpの端末でも無料ユーザーだけ50dp足りない状態で REGULAR の寸法が使われ、結果カードが先に潰れる（無料の360×640は実質584dp＝COMPACT相当）。条件は `useAds()` の `isBannerVisible` に1つだけ置き、バナー自身とこの判定の両方がそれを見る。**バナーのラッパーに `minHeight: CALCULATOR_BANNER_HEIGHT` を入れてあるのも同じ理由**——読み込み完了で50dp跳ねると、跳ねた後の高さで段階が変わってキーの高さまで変わる。
  - **結果の数字まわりの寸法も段階で持つ**（`showResultCardLabel` / `resultValueFontSize` / `resultValueMinHeight` / `resultCardPaddingVertical`）。見出し行（「結果」ラベル＋アイコン、約28dp）は**数字より上**に積まれるので、低い段階では行ごと畳んでアイコンを数字と同じ行の右端へ移す（`app/(tabs)/index.tsx` の `resultActionsRow` を2箇所で使い回す）。**数字は小さくしてでも1行を残す**（36→32→28→24）。分数（KaTeX）は行の倍の高さを使うので `stackedValueFontSize` で一緒に下げる。
  - **`middleMinHeight` は「結果の数字の行が入る高さ」。** カードの上半分はスクロールの外に固定してあるので（下の `app/(tabs)/index.tsx` の項）、ここを割ると数字そのものが切り取られる。値は `resultCardPaddingVertical + （見出し行 28）+ 2 + resultValueMinHeight` で、84 / 49 / 43 / 38。**下げるときはこの式ごと下げること**（`tests/calculator-layout.test.ts` が全段階で機械的に突き合わせる）。
  - **`keyboardPanelOpenByDefault`** — 式キーボードの既定パネル（「単位」）を最初から開くか。接頭語キー行＋レールで約70dp使い、DENSE以下ではそこがそのまま結果カードの表示域になるので畳んでおく（「単位」を押せば従来どおり開く）。**`useState` の初期値なので、あとから段階が変わっても勝手に開閉しない**（利用者が開いたものを閉じない）。
  - 検証は `npx expo export --platform web` + Playwright でビューポートの高さを変えて行う（**Webでは `fontScale` が常に1**なので、拡大そのものは再現できない。段階の分岐は高さで、係数の計算は `tests/calculator-layout.test.ts` で固定している）。**広告が出ないぶん、無料ユーザーの端末は測った高さより50dp低いと思って読むこと**（360×640の無料端末＝Webの360×584）。360×780/640/600/584/520/460/400 で、2行に折り返す長い式＋`ABC` パネルを開いた状態でもキーパッド下段がタブバーより上にあることを確認済み。
...
- `lib/notebook-formulas/` — プリセット計算ノートの中身。ビルド生成物は無く、`source/`配下のTypeScriptが唯一の情報源（旧`default-notebooks.json` + `scripts/generate-default-notebooks.ts`による生成ステップは廃止済み）。
  ...
  - **カテゴリグリッドの表示順は`PRESET_NOTEBOOK_CATEGORIES`の配列順そのまま**。親カテゴリとその子（`parentId`付き）は隣接させて並べる。現在は**最上位10枚**で、順は 理科（小・中）→ 高校物理 → 化学の量的関係 → 天体・宇宙 → 電気・エネルギー → 趣味・ものづくり → 暮らし → 資産運用・不動産 → 車・自転車 → 機械・構造設計（最後）。
  - **`source/finance.ts`（資産運用・不動産、2026-09-25・14件）は金額に単位を付けない。** 通貨はエンジンの単位ではないので、金額は裸の数値で既定値を `regionalDefault`（`investmentLumpSum` / `monthlyContribution` / `propertyPrice` / `monthlyRent` / `annualIncome` / `mortgageRate`）で端末の通貨に合わせる。
    - **日本円だけは万円単位**（`PRESET_PRICE_PROFILES.JPY` の 3000 = 3000万円）。円のままだと物件価格が必ず 1e7 を超え、`formatNumberForLocale` の境目で `3e+7` の指数表記になる。境目は電卓全体の表記なので金額のためには動かさない。説明文に6言語とも「日本円は万円単位」と書いてある。COP・CRC・TWD の物件価格は 1e7 を超えるが、万円に当たる単位が無いので指数表記を受け入れている。
    - **金額・金利・利回りの想定には `exact` を付ける。** 契約値・計画値は測定値ではなく、有効数字で丸めると月々の返済額が百の位で丸まってお金の計算として意味を失う。
    - **単位が無いので次元の検査が効かない**（`r/12` を `r*12` と書いてもエラーにならない）。`tests/finance-notebooks.test.ts` が一般的な答え（36万ドル・6.5%・30年 → 月2275.44）と突き合わせて式を固定している。物件価格と家賃は同じ物件の組（表面利回り2〜7%）であることも同じテストが全通貨で見る。
    - 期間の年数は無次元の回数で持つ（`(1+r)^n` の指数は無次元でなければならない）。年数を答えにする手順だけ `*1yr` を掛ける。
  ...
  - **呼び寸法は `exact` ではなく「桁を書き分けて」表す**（2026-09-22。利用者からの指示「有効数字を二桁にするために無理やり測定値でないとしている。素直に L=2.0m として」）。`L = 2m` に印を付けて桁を稼ぐと、**なぜその桁数なのかが式のどこにも書いていない**状態になる。`L = 2.00m` と書けば理由がその場に見える。**67件のうち寸法系41件から印を外し（残り26件）、代わりに値の桁を書き直した**（`45mm`→`45.0mm`・`5m`→`5.00m`・`2mm`→`2.00mm`）。狙う桁は**そのノートの説明文が言っている値がそのまま出る桁**にする——引張材ノートは `≈ 130 MPa（元 125 MPa）` と出ていて説明文の「125MPa＝降伏応力のちょうど半分」と食い違っていたが、`F=25.0kN`・`A=200mm^2`・`L=2.00m` にして **`125 MPa`（≈ なし）** になった。同様に木造床根太の `11.4 MPa`・IPE 200 の `161 MPa`・オイラー座屈の細長比 `154` が説明文と一致し、生の10桁が出ていた手順も消えた。
  - **測定していない定数に `exact: true` を付けるのは「数えた量」と「定義で決まる値」だけ**（2026-09-21に導入、2026-09-22に対象を狭めた）。残しているのは**個数・回数**（巻数・歯数・人数・塗り回数・量子数・自立日数）、**定義値**（標準重力 `g = 9.80665m/s^2`、オイラーの端末係数 `k = 1`、真空の屈折率 `n₁ = 1`）。「2.0個」とは書けないので、こちらは桁で表せない。
    - **例外として呼び寸法にも印が要るのは、加減算を通る2ノートだけ**（穴まわりの応力集中の `(w-d)`、中空軸の `D^4-d^4`）。測定値どうしの加減算は桁が読めないので（下の `lib/significant-figures.ts` の項）、桁をどう書いても丸めが丸ごと止まって `46.875 MPa`・`29.10261817 MPa` の生値に戻る。**この2つのシードにはその理由をコメントで残してある。**
    ...
    - 桁まわりを触った結果どう変わるかは **`pnpm notebook:rounding` を変更の前後で取って diff する**（`scripts/report-notebook-rounding.ts`）。**桁は型でもテストでも拾えず、エラーにもならずに変わる**ので、目視の突き合わせが唯一の検出手段。2026-09-21 の `exact` 導入時は380手順中55件、2026-09-22 の「桁で書き分ける」への移行では378手順中24件の表示が変わった。
...
- `app/(tabs)/index.tsx`（電卓） — **計算結果は state ではなく式から導出する**（`previewCalculatorInput`）。`=` を押さなくてもリアルタイムに結果が出る。`=` は「履歴に残す・定数を保存する・エラーを出す」確定操作だけを担当する。リアルタイム表示と確定計算は `lib/calculator-input.ts` の同じ関数（`evaluateCalculatorInput`）を通すこと（定数定義 `W = 3cm` の扱いが2箇所に分かれると、片方だけ値を出せない食い違いになる）。
  - **結果の数字はスクロールの外に固定する**（2026-09-23。「高さの少ない端末だと計算結果がスクロールしないと見えない」という報告）。`middle` は画面で唯一伸縮する場所なので、結果カードを丸ごと `ScrollView` へ入れると、カードの中で**数字より上にある見出し行（約28dp）**だけが残って数字が枠の外に出る。**実測（Web・360×460）で、比較表まで下へスクロールしたあとの数字の位置は `top: -75`**（画面の上へ完全に消える。400dpでは -99）。今はカードを上下に割り、**主表示（数字・診断・案内）までを固定部、単位チップから下と履歴・サンプルをスクロール部**にしてある。同じ条件で `top: 70`＝常に見える。
    - **この2つを1つの ScrollView に戻さないこと。** 枠と背景は同じ値（`resultCard` ＋ `resultCardHead` / `resultCardBody`）なので、見た目は従来どおり1枚のカード。境目の角丸と枠だけを落としてある。**下半分が無い場面**（診断・書きかけ・進数入力中）は上半分だけで角丸を閉じる。
    - **表示の分岐は `resultMode`（`base` / `value` / `diagnosis` / `incomplete` / `empty`）1つに閉じる。** 固定部とスクロール部で同じ条件を書き分けると必ずずれる（数字は出ているのに下が空状態のもの、という形になる）。
    - **`=` のエラー帯はカードの下。** 固定部より上へ置くと、いちばん見えなければならない数字がその帯のぶん枠の外へ押し出される。帯が出る場面は主表示が診断・空案内になっていることがほとんどなので、下半分が無く帯がすぐ下に来る。
    - **診断文だけは固定部の中でスクロールさせる**（`diagnosisScroll`・`resultCardShrink`。CodeRabbitが#84で🟡として検出）。主表示を固定部へ置いた以上、文が長いと固定部がそのぶん伸びて `middle` の `overflow: hidden` で**最後の行が切れる**（実測: 360×400・独語の次元不一致は4行で、最終行が欠けた）。数字と違って診断は「読むもの」なので、収まらないぶんはスクロールで受ける。
    - **その代わり `middleScroll` の `flexShrink` は 100。** 1 のままだと診断のとき固定部（`resultCardShrink`）と按分されて**両方が同時に縮み**、サンプルボタンの1行を残すために診断の最終行が切れる。大きくしておけば「二次的な導線（履歴・サンプル）を畳んでから固定部を縮める」順になる。360×460 では独語の4行が全部見え、360×400 でもスクロールで読み切れる。
    - **式が変わったらスクロールを先頭へ戻す**（`resultScrollRef`）。Androidの ScrollView は内容が縮んでも contentOffset をクランプしないので、比較表を開いて下まで見たあとに打ち直すとチップ列が画面の外に残る（単位レールの `railScrollKey`・サンプル一覧の `key` と同じ事象）。**表示単位を変えただけでは戻さない**——そのときは利用者が自分でその位置を見ている。
...
- `components/ui/expression-keyboard.tsx` + `lib/expression-keyboard.ts` — **電卓と計算ノート詳細が共用する式キーボード**（2026-09-19）。上から 開いているパネル → ツール行（`定数`・`xⁿ`・`f(x)`・`αβ`・`ABC`・`単位`・⌨）→ **5列4段**のキーパッド（`EXPRESSION_CELLS`）。**ツール行がパネルより下にあるのは意図的**（下の項を参照）。きっかけは「電卓とノートでキー配置が違う」「`f(x)` の列が接頭語の下に出るのがいまいち」「`x²` などを構造化して f(x) も上に」「接頭語と単位のセットを折りたたみたい」という一連の指摘。
  ...
  - **編集シート（`notebook-editor-sheet.tsx`）も 2026-09-22 からこの部品を使う**（下の項）。使っていないのはタイトル・説明文・数式・表示タイトル・表示単位の欄で、そこは従来どおり OS のキーボード（フォーカスが移ったらキーパッドを畳む）。
...
- `components/notebooks/notebook-keypad.tsx` + `lib/notebook-keypad.ts` — **計算ノート詳細の値欄に付くアプリ内キーパッド**（2026-09-18。2026-09-19 からは中身が電卓と共用の `ExpressionKeyboard` になり、このファイルは上段（欄名・閉じる）と記号・単位チップの列だけを足すラッパー）。きっかけは「計算ノートでキーボードが出ず入力できない」という実機報告。値欄の入力手段が OS のキーボードだけだったため、それが上がらない端末（電卓の隠し TextInput と同じ現象。**原因は依然として未特定**）では値を一切変えられなかった。
  ...
  - **キーを押しても欄のフォーカスを外さないこと**（2026-09-25。「`◀ ▶` が効かない・カーソルが見えない」と報告された）。フォーカスが外れた欄は `selection` prop で位置を動かしてもキャレットを描かないので、内部の位置は動いているのに画面では何も起きていないように見える。Web はキー（Pressable）が mousedown でフォーカスを奪うので、`NotebookKeypad` の root で `onMouseDown` の既定動作を止めている（RNW の View は `onMouseDown` を DOM へ渡す）。加えてキーの操作のあとに `keepActiveFieldFocused` が欄へ `focus()` し直す（ネイティブで外れた場合の保険。欄は `showSoftInputOnFocus=false` なので OS のキーボードは上がらない）。詳細画面と編集シートの両方。
  ...
  - **編集シート（`notebook-editor-sheet.tsx`）も同じ `NotebookKeypad` を使う**（2026-09-22。利用者からの指示「ローカル定数や手順の入力時には、テキストボックスの下のボタンではなく他と共通のアプリキーパッドを」）。**欄の直下に出していた記号レール（ギリシャ文字・下付き）と「定義済みの変数／単位」のチップ列は廃止。** 数字と演算子は端末のキーボード・記号と単位は欄の下、という分かれ方で、電卓ともノート詳細ともキーの位置と中身が違っていた（単位はカテゴリも文脈依存の候補も接頭語の補完も無い平らな一覧だった）。
    - **この画面だけは「名前＝式」を丸ごと1本のテキストとして編集する**（`lib/notebook-keypad.ts` の `insertInCombinedField` / `backspaceInCombinedField` / `moveCaretInCombinedField`）。ここは**名前そのものを作る画面**で、`σ_y`・`mₒ` は端末のキーボードでは打てないため、記号パネルから名前側へ入れられないと作れない。詳細画面は既存の名前の「値」だけを編集するので、あちらの `insertKeypadText` は逆に名前を守る。**同じ欄でも役割が違うので関数を分けてある**（片方に寄せると必ずどちらかが壊れる）。接頭語キーと単位チップだけは式の座標で動くので、`mapCombinedSelectionToExpressionRange` で直してから純関数へ渡す。
    - **シートの `ScrollView` に `flexShrink: 1` を明示すること**（`styles.body`）。RN の既定は 0 なので、書かないとキーパッドがシートの外へ押し出されて**画面に出ない**（実際に踏んだ。単位レールの `startRailWrap` と同じ事象）。
    - 「名前＝式」以外の欄は `onFocus` で `activeRailKey` を null にしてキーパッドを畳む。**これが無いと OS のキーボードとキーパッドが同時に積まれて入力欄が残らない。**
    - ライブラリの定数欄（グローバル定数）は `ConstantEditorSheet` が既に同じ部品を使っている。
...
40. **[完了]** **計算ノートの編集シートを電卓と同じアプリ内キーパッドにし、プリセットの呼び寸法から `exact` を外して桁で書き分けた。** どちらも利用者からの指示。前者は「ローカル定数や手順の入力時には、テキストボックスの下のボタンではなく他と共通のアプリキーパッドを」で、欄ごとの記号・変数・単位のチップ列を廃止してシート下端に `NotebookKeypad`（電卓・ノート詳細と共用）を出す形にした。この画面だけは**名前も式も1本のテキストとして編集する**（ギリシャ文字の名前を作れる必要があるため。`lib/notebook-keypad.ts` の `insertInCombinedField` ほか）。後者は「有効数字を二桁にするために無理やり測定値でないとしている。素直に L=2.0m として」で、41件の印を外し `L=2.00m` のように意図した桁を書いた結果、引張材の軸応力が説明文どおりの `125 MPa` になった（378手順中24件の表示が変わった）。

41. **[完了]** **結果の数字をスクロールの外へ固定した。** 「高さの少ないスマホ端末だと、計算結果がスクロールしないと見えないときがある」という報告。原因は3つ重なっていた: (1) 結果カードが丸ごと縦スクロールの中にあり、しかもカードの中で**数字より上に見出し行（約28dp）**がある、(2) `middleMinHeight` が段階ごとに 56/44/36/24dp で**数字1行（46dp）より小さい**＝「数字が見える」保証が仕様上なかった、(3) **Webでは `CalculatorBannerAd` が描かれない**ので、このファイルの Playwright 実測値はすべて無料ユーザーの50dpを含んでいなかった。カードを上下に割って主表示までを固定部にし、見出し行は低い段階で畳んでアイコンを数字の行へ移し、数字の大きさ・`middleMinHeight` も段階で持つようにした。あわせてバナーのぶんを段階の判定へ入れ、DENSE以下では式キーボードの単位パネルを既定で畳み、式が変わったらカード下半分のスクロールを先頭へ戻すようにした。

### 現在の基準値（2026-09-23時点、結果の数字をスクロールの外へ固定した後）

- `npx tsc --noEmit` → **`app/_layout.tsx` の `@/global.css` で1件のみ**（従来どおりの環境依存）。
- `npx vitest run` → **1184 passed / 2 failed**。失敗2件は従来どおり `tests/revenuecat.credentials.test.ts`（環境依存）。`tests/calculator-layout.test.ts` が 12→17件。
- `npx expo lint` → **2エラー・0警告**（`app/(tabs)/index.tsx` の既存分のまま）。
- `npx expo export --platform web` が通る（**初回は `react-native-css-interop/.cache/web.css` のSHA-1で落ちることがあり、同じコマンドをもう一度走らせると通る**）。Playwright（ja・light）で実測:
  - **結果の数字の位置**（`12V / 4.7kΩ` → `2.55319 mA`。カード下半分を下端までスクロールした状態）: 360×640 で `top 111`、584 で 79、520 で 74、460 で 70、400 で 70。**`ABC` パネルを開いても同じ値**（固定部なので動かない）。**変更前は 460 で `top -75`・400 で `-99`＝画面の外**だった。
  - **キーパッド下段とタブバー**: 780＝678.5/748、640＝538.5/608、584＝492.5/552、600＝508.5/568、520＝430.5/488、460＝372.5/428、400＝312.5/368。**2行に折り返す長い式＋`ABC` パネルを開いた状態でも同じ**（どれもキーパッドが上）。
  - 式を打ち直すとカード下半分のスクロールが先頭へ戻る（360×460 で `scrollTop` 68 → 0）。
- **未検証（この環境では Web しか動かせない）**: 端末の文字サイズを上げたときの段階の切り替わり（Webでは `fontScale` が常に1）、バナー広告が載ったときの実際の高さ（Webでは広告そのものが出ない）。**Android 実機で確認すること。**
