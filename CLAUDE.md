...
- **各言語版ターゲットに沿った施策（`docs/target-users-by-locale-2026-09.md` 第5節）**: P0=計量カップの既定を言語ではなくロケールから決める・掲載文とASOキーワードを言語ごとに書き分ける（英語からの直訳をやめる）、P1=PS/CV の追加・燃費の単位グループ・中南米/台湾の価格プロファイル、P2=分数インチとAWG（**米国の職人を取ると決めた場合のみ**。単体では入れない）。
