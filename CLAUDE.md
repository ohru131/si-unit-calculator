...
- `components/ui/expression-keyboard.tsx` + `lib/expression-keyboard.ts` — **電卓と計算ノート詳細が共用する式キーボード**（2026-09-19）。上から 開いているパネル → ツール行（`定数`・`f(x)`・`αβ`・`ABC`・`単位`・⌨）→ **6列4段**のキーパッド（`EXPRESSION_KEYS`）。**ツール行がパネルより下にあるのは意図的**（下の項を参照）。きっかけは「電卓とノートでキー配置が違う」「`f(x)` の列が接頭語の下に出るのがいまいち」「`x²` などを構造化して f(x) も上に」「接頭語と単位のセットを折りたたみたい」という一連の指摘。
  ...
  - **パネルの中身**: `powers`＝`POWER_KEYS`（x² x³ xʸ ×10ⁿ）、`functions`＝`MATH_FUNCTION_KEYS`、`symbols`＝`FORMULA_CHARACTER_GROUPS`（編集シートの記号レールと同じデータ。グループのタブ行＋文字のグリッド）、`alphabet`＝`ALPHABET_ROWS`（QWERTY 3段・`⇧` は1文字ぶんの大文字化・`_`）、`units`＝`PREFIX_KEYS` の行＋親が渡す `unitPanel`（電卓は `renderUnitRail(true)`、ノートは記号・単位チップ）。**`ABC` パネルを足した理由**は、日本語IMEのまま端末のキーボードで打つと全角の `ｍ` が入って単位として認識されないため（`lib/fullwidth-input.ts` の `toHalfWidthAscii` で受け口側も半角へ揃えてある。`String.prototype.normalize` は使わない）。
  ...
  - ノートは `submitAsDone` で `=` の位置に ✓（確定＝キーパッドを畳む）を出し、AC は式を空にする。名前側（`m=`）はキーパッドの編集対象外（`moveCaretInField` も式の範囲だけを動く）。
