...
- `components/ui/expression-keyboard.tsx` + `lib/expression-keyboard.ts` — **電卓と計算ノート詳細が共用する式キーボード**（2026-09-19）。上から ツール行（`< >`・`xⁿ`・`f(x)`・`αβ`・`ABC`・`単位`・⌨）→ 開いているパネル → 5列4段のキーパッド（`EXPRESSION_KEYS`）。きっかけは「電卓とノートでキー配置が違う」「`f(x)` の列が接頭語の下に出るのがいまいち」「`x²` などを構造化して f(x) も上に」「接頭語と単位のセットを折りたたみたい」という一連の指摘。
  ...
  - **パネルの中身**: `powers`＝`POWER_KEYS`（x² x³ xʸ ×10ⁿ）、`functions`＝`MathFunctionRail`、`symbols`＝`FORMULA_CHARACTER_GROUPS`（編集シートの記号レールと同じデータ。タブ＋チップの2行）、`alphabet`＝`ALPHABET_ROWS`（QWERTY 3段・`⇧` は1文字ぶんの大文字化・`_`）、`units`＝`PREFIX_KEYS` の行＋親が渡す `unitPanel`（電卓は `renderUnitRail(true)`、ノートは記号・単位チップ）。**`ABC` パネルを足した理由**は、日本語IMEのまま端末のキーボードで打つと全角の `ｍ` が入って単位として認識されないため（`lib/fullwidth-input.ts` の `toHalfWidthAscii` で受け口側も半角へ揃えてある。`String.prototype.normalize` は使わない）。
  ...
  - `layout`（`CalculatorLayout`）を受け取り、`scaleFontSizes` でキーの高さ・文字の拡大率を段階に追従させる。ノートも `resolveCalculatorLayout` で同じ段階を使う。360×640 で `=` の下端 538.5（どのパネルを開いても同じ。`middle` が吸収）。
...
- `components/notebooks/notebook-keypad.tsx` + `lib/notebook-keypad.ts` — **計算ノート詳細の値欄に付くアプリ内キーパッド**（2026-09-18。2026-09-19 からは中身が電卓と共用の `ExpressionKeyboard` になり、このファイルは上段（欄名・閉じる）と記号・単位チップの列だけを足すラッパー）。きっかけは「計算ノートでキーボードが出ず入力できない」という実機報告。値欄の入力手段が OS のキーボードだけだったため、それが上がらない端末（電卓の隠し TextInput と同じ現象。**原因は依然として未特定**）では値を一切変えられなかった。
  ...
  - **数学関数は上段の `f(x)` キーで開く横スクロールのチップ列**（`NOTEBOOK_KEYPAD_FUNCTIONS`。電卓の `ADVANCED_KEYS` と同じ並びで `^` だけ除く）。キーパッド本体の20キーには混ぜない（配置を電卓と揃えるため）。開閉状態はコンポーネント内の `useState` に閉じる。テストが全項目を実エンジンで評価できることを固定している。
