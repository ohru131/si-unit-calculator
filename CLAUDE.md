...
- `npx vitest run` → **819 passed / 2 failed / 1 skipped**。失敗2件は従来どおり `tests/revenuecat.credentials.test.ts`（環境依存）。新規: `tests/locale-defaults.test.ts`（10件）・`tests/sample-calculations.test.ts`（7件）、`tests/preset-regional-defaults.test.ts` に6件、`tests/units.test.ts` に PS/CV と燃費のブロック。
- `npx expo lint` → **2エラー・0警告**（`app/(tabs)/index.tsx` の既存分のまま）。
...
- **投入済みプリセットの後追い更新**。シードを直しても既存インストールに届かないので、`applyPresetResultSymbols` と同じ仕組み（`PRESET_NOTEBOOK_SEEDS_AS_SEEDED` と突き合わせ、**ユーザーが編集していないものだけ**差し替える）を定数・式にも広げる案。#46 で作り直した「はり・柱」も、既存ユーザーには旧・材料力学のまま残っている。**#54 の地域別燃費も同じ穴に落ちている**（`regionalDefault` は投入時にしか効かないので、既存ユーザーの走行コストノートは米国・英国でも `15km/L` のまま。CodeRabbitが🟠Majorとして検出したが、移行処理は `lib/calculator-store.tsx` に手を入れる別作業なので#54には入れていない）。**次にやるならこの項目の第一候補。**
