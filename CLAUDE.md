...
- `components/notebooks/notebook-keypad.tsx` + `lib/notebook-keypad.ts` — **計算ノート詳細の値欄に付くアプリ内キーパッド**（2026-09-18）。きっかけは「計算ノートでキーボードが出ず入力できない」という実機報告。値欄の入力手段が OS のキーボードだけだったため、それが上がらない端末（電卓の隠し TextInput と同じ現象。**原因は依然として未特定**）では値を一切変えられなかった。
  ...
  - **報告の真の原因は「キーボードが上がらない」ではなく、手順の式欄が入力欄に見えず当たり判定も狭かったこと**（利用者が後から「薄い文字のところを押すとキーボードが出た」と気付いた）。旧 `resultExpressionInput` は12pxの文字＋下線だけで高さ約20px。定数欄と同じ枠付き・`minHeight: 42` の箱に変え、6言語のプレースホルダも足した。**入力欄は枠と高さで「押せる場所」を見せること**——下線だけの欄は他の画面にも作らない。キーパッド自体は電卓との一貫性のために残してある（OS のキーボードをタップで出す既定に戻したい場合は `showSoftInputOnFocus` を外すだけ）。
