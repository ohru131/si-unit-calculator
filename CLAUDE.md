...
- `lib/notebook-export-model.ts` / `lib/notebook-export-html.ts` / `lib/notebook-export.ts` — 計算ノートのPDFエクスポート（Pro機能）。
  ...
  - **既定を有効数字にした影響は小さくない。** 実測（2026-09-21）でプリセット365手順のうち**197手順（54%）が丸められ、196手順が有効1桁**になる。`t=2h`・`t₂=3h` のような「数える量」も1桁として数えるため、理科の速さのノートが `210 km → ≈ 200 km`・`3.5 h → ≈ 4 h` になる。規則としては正しく、丸める前の値も併記されるが、**気になるならシードの定数を `2.0h` と書くか、桁が1のときは丸めないという例外を入れる**（現状は例外なし）。
